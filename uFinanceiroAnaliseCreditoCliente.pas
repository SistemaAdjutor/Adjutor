...
uses
  DataCad, RWFunc, uFinanceiroAnaliseCreditoConfirma, Men0001, Cli0002;
...
begin
   sQuery := 'SELECT '+
             ...
             'coalesce((SELECT SUM(T9.ped_vltotal_liq) FROM ped0000 T9 WHERE T9.cli_codigo = T1.cli_codigo '+iif(chkMultiEmpresa.Checked,'',' and t9.emp_codigo = '+QuotedStr(FrmMenu.CAD_EMPRESA.EMP_CODIGO))+' AND '+
             'T9.ped_situacao = ''F'' AND T9.opv_codigo IN (SELECT T10.opv_codigo FROM opv0000 T10 WHERE T10.opv_venda = ''S'')),0) AS PEDIDOS_ABERTOS, '+
             'coalesce((SELECT SUM(T9.ped_vltotal_liq) FROM ped0000 T9 WHERE T9.cli_codigo = T1.cli_codigo '+iif(chkMultiEmpresa.Checked,'',' and t9.emp_codigo = '+QuotedStr(FrmMenu.CAD_EMPRESA.EMP_CODIGO))+' AND T9.ped_situacao = ''F'' '+
             'AND T9.opv_codigo IN (SELECT T10.opv_codigo FROM opv0000 T10 WHERE T10.OPV_ORCAMENTO = ''S'')),0) AS PEDIDOS_ORCAMENTOS, '+
             'coalesce((SELECT MAX(T9.ped_vltotal_liq) FROM ped0000 T9 WHERE T9.cli_codigo = T1.cli_codigo '+iif(chkMultiEmpresa.Checked,'',' and t9.emp_codigo = '+QuotedStr(FrmMenu.CAD_EMPRESA.EMP_CODIGO))+' AND T9.opv_codigo IN (SELECT '+
...
begin                
   sCondicao := '';
   if (FrmMenu.CAD_EMPRESA.bVisualizaClientes_P) and (Frmmenu.wACESSOS_USUARIO.CODIGO_REPRES <> '000') then
       sCondicao := ' WHERE REP_CODIGO ='''+Frmmenu.wACESSOS_USUARIO.CODIGO_REPRES+'''';
   //Combo
   CbCliente.CodigoEmpresa := StrToInt(FrmMenu.CAD_EMPRESA.EMP_CODIGO);
   CbCliente.Compartilhar := 'CLIENTES';
   if (sCondicao <> '') then
      CbCliente.Condicao :=  'REP_CODIGO ='''+Frmmenu.wACESSOS_USUARIO.CODIGO_REPRES+'''';
