...
type
 ...
 TSgDbSearchComboFDAC = class(tCustomEdit)
...
Procedure Register;
begin
 RegisterComponents('Infosaga', [TSgDbSearchComboFDAC]);
end;

{$R saga.res}
{ TSgDbSearchComboFDAC }

procedure TSgDbSearchComboFDAC.CMBidimodechanged(var Message: TMessage);
begin
 inherited;
...
procedure TSgDbSearchComboFDAC.CMEnabledchanged(var Message: TMessage);
begin
 inherited;
 if vlButton <> nil then
...
procedure TSgDbSearchComboFDAC.CMEnter(var Message: TCMEnter);
begin
 inherited;
...
procedure TSgDbSearchComboFDAC.CMVisiblechanged(var Message: TMessage);
begin
 inherited;
 if vlButton <> nil then
 begin
...
constructor TSgDbSearchComboFDAC.Create(AOwner: TComponent);
var
 vBitmap: tBitMap;
begin
  vAtivarFiltro:= False;
...
procedure TSgDbSearchComboFDAC.Notification(AComponent: TComponent;
...
procedure TSgDbSearchComboFDAC.Resize;
begin
...
procedure TSgDbSearchComboFDAC.SetAutoF8(const Value: TsgDBAutoF8);
begin
 vlAutoF8 := Value;
end;

procedure TSgDbSearchComboFDAC.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
 P: tPoint;
begin
...
procedure TSgDbSearchComboFDAC.SetButton(const value: tSpeedButton);
begin
  vlButton:= Value;
end;

procedure TSgDbSearchComboFDAC.SetDataField(const Value: String);
begin
...
procedure TSgDbSearchComboFDAC.DataLinkChange(Sender: TObject);
var
 NewID: string;
begin
...
procedure TSgDbSearchComboFDAC.SetDataSource(const Value: tDataSource);
begin
...
procedure TSgDbSearchComboFDAC.SetEmptyText(const Value: String);
begin
 vlEmptyText := Value;
end;

Procedure TSgDbSearchComboFDAC.MontaSqlById;
begin
...
Procedure TSgDbSearchComboFDAC.MontaSqlByIdNulo;
begin
 // if  not (csLoading in ComponentState) then
 begin
...
procedure TSgDbSearchComboFDAC.ShowErrRegistroNaoLocalizado;
var
 flgerr: Boolean;
begin
 if (fDataFieldFormat <> '') then
...
procedure TSgDbSearchComboFDAC.SetIdRetorno(const Value: string);
var
...
procedure TSgDbSearchComboFDAC.SetLookupDispl(const Value: string);
begin
 vlLookupDispl := Value.ToUpper;
end;

procedure TSgDbSearchComboFDAC.SetLookupKeyField(const Value: string);
begin
 vlLookupKeyField := Value;
end;

procedure TSgDbSearchComboFDAC.SetLookupTableShare(const Value: string);
begin
...
procedure TSgDbSearchComboFDAC.SetParent(AParent: tWinControl);
begin
...
procedure TSgDbSearchComboFDAC.SetShowButton(const Value: Boolean);
begin
...
procedure TSgDbSearchComboFDAC.SetVlQry(const Value: TFDQuery);
begin
 vlQry := Value;
 vlDS.DataSet := vlQry;
 vlDS.AutoEdit := False;
...
procedure TSgDbSearchComboFDAC.TestarSeAindaExiste;
var
 vqry: TFDQuery;
begin
 HideGrid;
...
function TSgDbSearchComboFDAC.GetParentada(pComponent: tWinControl): string;
begin
...
procedure TSgDbSearchComboFDAC.DoAutoF8;
var
...
procedure TSgDbSearchComboFDAC.DefaultButtonClick(Sender: TObject);
begin
 if not(csDesigning in ComponentState) then
 begin
...
destructor TSgDbSearchComboFDAC.Destroy;
begin
...
procedure TSgDbSearchComboFDAC.Clear;
begin
 inherited Clear;
 idRetorno := '';
end;

procedure TSgDbSearchComboFDAC.Click;
begin
 inherited;
 self.selectall;
end;

procedure TSgDbSearchComboFDAC.HideGrid;
begin
 if not(csDesigning in ComponentState) then
 begin
  if assigned(vlGrid) then
...
function TSgDbSearchComboFDAC.GetAutoF8: TsgDBAutoF8;
begin
 result := vlAutoF8;
end;


function TSgDbSearchComboFDAC.GetCds: TFDQuery;
begin
  result := vlQry
end;

function TSgDbSearchComboFDAC.GetDataField: String;
begin
 result := vlDataLink.FieldName;
end;

function TSgDbSearchComboFDAC.GetDataSource: tDataSource;
begin
...
function TSgDbSearchComboFDAC.GetField: TField;
begin
 if vlDataLink <> Nil then
  if assigned(vlDataLink) then
...
function TSgDbSearchComboFDAC.GetFormParent: tForm;
var
 vlCmp: tWinControl;
begin
...
Procedure TSgDbSearchComboFDAC.CalculaCoordenadasFormParent(var pPonto: tPoint);
var
 pRect: TRect;
 vlCmp: tWinControl;
 x, y: Integer;
...
procedure TSgDbSearchCombofdac.DBGridDrawColumnCell(Sender: TObject;
...
procedure TSgDbSearchComboFDAC.ShowGrid;
var  vData: String;
     vPixWidth, x: Integer;
     vPonto: tPoint;
     nm: string;
...
procedure TSgDbSearchComboFDAC.GridClick(Sender: TObject);
var
 nm: string;
begin
...
Procedure TSgDbSearchComboFDAC.AtribSql(pDset: tSqlQuery);
begin
...
Function TSgDbSearchComboFDAC.MontaWhere: string;
var
...
Procedure TSgDbSearchComboFDAC.MontaSql;
begin
 // if  not (csLoading in ComponentState) then
 begin
//  vlCDS.PacketRecords := 4;
...
procedure TSgDbSearchComboFDAC.KeyDown(var Key: Word; Shift: TShiftState);
var
 lKey: Char;
begin
...
procedure TSgDbSearchComboFDAC.SetDataLinkValue(pId: string);
begin
 if not(csDesigning in ComponentState) then
...
Function TSgDbSearchComboFDAC.GetIdByText(const Value: string): String;
var
 vqry: TFDQuery;
 vDesc: String;
begin
...
function TSgDbSearchComboFDAC.GetIdRetorno: string;
begin
 result := trim(self.vlIdRetorno);
end;

function TSgDbSearchComboFDAC.GetSql: String;
begin
 if assigned(vlQry) then
  result := vlQry.Sql.Text;
end;

procedure TSgDbSearchComboFDAC.FechaTudo;
begin
 vlGridClick := False;
 HideGrid;
 // vlCds.Close;
...
procedure TSgDbSearchComboFDAC.CMExit(var Message: TCMExit);
///
begin
...
procedure TSgDbSearchComboFDAC.CMMouseWhell(var Message: TCMMouseWheel);
var
 n: Integer;
begin
...
Procedure TSgDbSearchComboFDAC.TmrOnTimer(Sender: TObject);
begin
 if not(csDestroying in ComponentState) then
 begin
  vlTimer.Enabled := False;
...
procedure TSgDbSearchComboFDAC.Change;
begin
 inherited;
 if not(csDesigning in ComponentState) then
 begin
...
procedure TSgDbSearchComboFDAC.TestarErrosAdjutor;
begin
...
Function TSgDbSearchComboFDAC.GetFiltroEmpresa: string;
VAR
 acron: STRING;
begin
...
procedure TSgDbSearchComboFDAC.Filtro;
begin
 if not(csDesigning in ComponentState)  then
 begin
...
Procedure TSgDbSearchComboFDAC.CdsAfterOpen(DataSet: TDataSet);
var
 a, n: Integer;
 nomes: string;
begin
...
procedure TSgDbSearchComboFDAC.FldGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
var
 fld: TField;
begin
...
procedure TSgDbSearchComboFDAC.SetName(const Value: TComponentName);
var
 LClearText: Boolean;
begin
...
procedure TSgDbSearchComboFDAC.KeyPress(var Key: Char);
begin
...
procedure TSgDbSearchComboFDAC.CreateAutoF8;
var
 dAutoF8: TsgDBAutoF8;
