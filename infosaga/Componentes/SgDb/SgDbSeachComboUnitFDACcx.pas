...
uses Classes, StdCtrls, Buttons, ExtCtrls, SqlExpr, Provider, DBClient,
 ...
 FireDAC.Stan.Def, FireDAC.Stan.Pool, FireDAC.Phys, FireDAC.Phys.FB, cxgridCustomTableView, cxGraphics,
 FireDAC.Comp.Client, FireDAC.Comp.UI, FireDAC.Phys.IBBase, FireDAC.Comp.DataSet,
 Vcl.DBGrids, SgDbAutoF8Unit, cxGrid, cxGridLevel,cxGridDBTableView;

type

 TBeforeSelectedEvent = procedure(Sender: TObject;  text: TCaption) of object;
 tAGrid = class(TcxGrid);

 TSgDbSearchComboFDACcx = class(tCustomEdit)
  procedure TmrOnTimer(Sender: TObject);
  procedure GridClick(Sender: TObject);
 
  procedure CustomDrawCell(Sender: TcxCustomGridTableView; ACanvas: TcxCanvas; AViewInfo:
...
Procedure Register;
begin
 RegisterComponents('Infosaga', [TSgDbSearchComboFDACcx]);
end;

{$R saga.res}
{ TSgDbSearchComboFDACcx }

procedure TSgDbSearchComboFDACcx.CMBidimodechanged(var Message: TMessage);
begin
 inherited;
...
procedure TSgDbSearchComboFDACcx.CMEnabledchanged(var Message: TMessage);
begin
 inherited;
 if vlButton <> nil then
...
procedure TSgDbSearchComboFDACcx.CMEnter(var Message: TCMEnter);
begin
 inherited;
...
procedure TSgDbSearchComboFDACcx.CMVisiblechanged(var Message: TMessage);
begin
 inherited;
 if vlButton <> nil then
 begin
...
constructor TSgDbSearchComboFDACcx.Create(AOwner: TComponent);
var
 vBitmap: tBitMap;
begin
  vAtivarFiltro:= False;
...
procedure TSgDbSearchComboFDACcx.Notification(AComponent: TComponent;
...
procedure TSgDbSearchComboFDACcx.Resize;
begin
...
procedure TSgDbSearchComboFDACcx.SetAutoF8(const Value: TsgDBAutoF8);
begin
 vlAutoF8 := Value;
end;

procedure TSgDbSearchComboFDACcx.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
 P: tPoint;
begin
...
procedure TSgDbSearchComboFDACcx.SetButton(const value: tSpeedButton);
begin
  vlButton:= Value;
end;

procedure TSgDbSearchComboFDACcx.SetDataField(const Value: String);
begin
...
procedure TSgDbSearchComboFDACcx.DataLinkChange(Sender: TObject);
var
 NewID: string;
begin
...
procedure TSgDbSearchComboFDACcx.SetDataSource(const Value: tDataSource);
begin
...
procedure TSgDbSearchComboFDACcx.SetEmptyText(const Value: String);
begin
 vlEmptyText := Value;
end;

Procedure TSgDbSearchComboFDACcx.MontaSqlById;
begin
...
Procedure TSgDbSearchComboFDACcx.MontaSqlByIdNulo;
begin
 // if  not (csLoading in ComponentState) then
 begin
...
procedure TSgDbSearchComboFDACcx.ShowErrRegistroNaoLocalizado;
var
 flgerr: Boolean;
begin
 if (fDataFieldFormat <> '') then
...
procedure TSgDbSearchComboFDACcx.SetIdRetorno(const Value: string);
var
...
procedure TSgDbSearchComboFDACcx.SetLookupDispl(const Value: string);
begin
 vlLookupDispl := Value.ToUpper;
end;

procedure TSgDbSearchComboFDACcx.SetLookupKeyField(const Value: string);
begin
 vlLookupKeyField := Value;
end;

procedure TSgDbSearchComboFDACcx.SetLookupTableShare(const Value: string);
begin
...
procedure TSgDbSearchComboFDACcx.SetParent(AParent: tWinControl);
begin
...
procedure TSgDbSearchComboFDACcx.SetShowButton(const Value: Boolean);
begin
...
procedure TSgDbSearchComboFDACcx.SetVlQry(const Value: TFDQuery);
begin
 vlQry := Value;
 vlDS.DataSet := vlQry;
 vlDS.AutoEdit := False;
...
procedure TSgDbSearchComboFDACcx.TestarSeAindaExiste;
var
 vqry: TFDQuery;
begin
 HideGrid;
...
function TSgDbSearchComboFDACcx.GetParentada(pComponent: tWinControl): string;
begin
...
procedure TSgDbSearchComboFDACcx.DoAutoF8;
var
...
procedure TSgDbSearchComboFDACcx.DefaultButtonClick(Sender: TObject);
begin
 if not(csDesigning in ComponentState) then
 begin
...
destructor TSgDbSearchComboFDACcx.Destroy;
begin
...
procedure TSgDbSearchComboFDACcx.Clear;
begin
 inherited Clear;
 idRetorno := '';
end;

procedure TSgDbSearchComboFDACcx.Click;
begin
 inherited;
 self.selectall;
end;

procedure TSgDbSearchComboFDACcx.HideGrid;
begin
 if not(csDesigning in ComponentState) then
 begin
  if assigned(vlGrid) then
...
function TSgDbSearchComboFDACcx.GetAutoF8: TsgDBAutoF8;
begin
 result := vlAutoF8;
end;


function TSgDbSearchComboFDACcx.GetDataField: String;
begin
 result := vlDataLink.FieldName;
end;

function TSgDbSearchComboFDACcx.GetDataSource: tDataSource;
begin
...
function TSgDbSearchComboFDACcx.GetField: TField;
begin
 if vlDataLink <> Nil then
  if assigned(vlDataLink) then
...
function TSgDbSearchComboFDACcx.GetFormParent: tForm;
var
 vlCmp: tWinControl;
begin
...
Procedure TSgDbSearchComboFDACcx.CalculaCoordenadasFormParent(var pPonto: tPoint);
var
 pRect: TRect;
 vlCmp: tWinControl;
 x, y: Integer;
...
procedure TSgDbSearchComboFDACcx.ShowGrid;
var  vData: String;
     vPixWidth, x: Integer;
     vPonto: tPoint;
     nm: string;
         ...
         begin
              ...
              begin
                   ...
                   try
                       ...
                       vlGrid :=  TcxGrid.Create(GetFormParent);
                       vlGrid.Parent := GetFormParent;
                       //  vlGrid.ControlStyle := vlGrid.ControlStyle + [csClickEvents];
                       // Creates a Level
                       Level := vlGrid.Levels.Add;
//                       Level.Parent //:= GetFormParent;
                       Level.Name := 'Nivelpesquisa';

                       // Creates a View
                       View := TcxGridDBTableView(vlGrid.CreateView(TcxGridDBTableView));
//                       View := vlGrid.CreateView(TcxGridDBTableView) as TcxGridDBTableView;

                       /// ... and binds it to the Level
                       Level.GridView := View;
                       View.FilterRow.Visible := False;
                       view.OnCustomDrawCell := CustomDrawCell;
                       View.Name := 'PesquisaCombo';
                       view.DataController.DataSource := vlDS;
                       view.OptionsData.Appending := False;
                       view.OptionsData.CancelOnExit := False;
                       view.OptionsData.Deleting := False;
                       view.OptionsData.DeletingConfirmation := False;
...
procedure TSgDbSearchComboFDACcx.GridClick(Sender: TObject);
var
 nm: string;
begin
...
Procedure TSgDbSearchComboFDACcx.AtribSql(pDset: tSqlQuery);
begin
...
Function TSgDbSearchComboFDACcx.MontaWhere: string;
var
...
Procedure TSgDbSearchComboFDACcx.MontaSql;
begin
 // if  not (csLoading in ComponentState) then
 begin
//  vlCDS.PacketRecords := 4;
...
procedure TSgDbSearchComboFDACcx.KeyDown(var Key: Word; Shift: TShiftState);
var
 lKey: Char;
begin
...
procedure TSgDbSearchComboFDACcx.SetDataLinkValue(pId: string);
begin
 if not(csDesigning in ComponentState) then
...
Function TSgDbSearchComboFDACcx.GetIdByText(const Value: string): String;
var
 vqry: TFDQuery;
 vDesc: String;
begin
...
function TSgDbSearchComboFDACcx.GetIdRetorno: string;
begin
 result := trim(self.vlIdRetorno);
end;

function TSgDbSearchComboFDACcx.GetSql: String;
begin
 if assigned(vlQry) then
  result := vlQry.Sql.Text;
end;

procedure TSgDbSearchComboFDACcx.FechaTudo;
begin
 vlGridClick := False;
 HideGrid;
 // vlCds.Close;
...
procedure TSgDbSearchComboFDACcx.CMExit(var Message: TCMExit);
///
begin
...
procedure TSgDbSearchComboFDACcx.CMMouseWhell(var Message: TCMMouseWheel);
var
 n: Integer;
begin
...
Procedure TSgDbSearchComboFDACcx.TmrOnTimer(Sender: TObject);
begin
 if not(csDestroying in ComponentState) then
 begin
  vlTimer.Enabled := False;
...
procedure TSgDbSearchComboFDACcx.Change;
begin
 inherited;
 if not(csDesigning in ComponentState) then
 begin
...
procedure TSgDbSearchComboFDACcx.TestarErrosAdjutor;
begin
 ...
 begin
  ...
  if assigned(dbInicio) then
   if assigned(vlQry) then
    ...
    begin
//     if not vlQry.Connection.Connected then
//      vlQry.Connection.Params := dbInicio.Maindb.Params;
    end
    else
...
Function TSgDbSearchComboFDACcx.GetFiltroEmpresa: string;
VAR
 acron: STRING;
begin
...
procedure TSgDbSearchComboFDACcx.Filtro;
begin
 if not(csDesigning in ComponentState)  then
 begin
...
Procedure TSgDbSearchComboFDACcx.CdsAfterOpen(DataSet: TDataSet);
var
 a, n: Integer;
 nomes: string;
begin
...
procedure TSgDbSearchComboFDACcx.FldGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
var
 fld: TField;
begin
...
procedure TSgDbSearchComboFDACcx.SetName(const Value: TComponentName);
var
 LClearText: Boolean;
begin
...
procedure TSgDbSearchComboFDACcx.KeyPress(var Key: Char);
begin
...
procedure TSgDbSearchComboFDACcx.CreateAutoF8;
var
 dAutoF8: TsgDBAutoF8;
...
procedure TSgDbSearchComboFDACcx.CustomDrawCell(Sender: TcxCustomGridTableView; ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
begin
  If AViewInfo.GridRecord.RecordIndex mod 2 = 0 Then
       ACanvas.Brush.Color := $00FFEFDF // azul claro //
    Else
