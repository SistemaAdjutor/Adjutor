...
SET VERSAOSISTEMA.SCRIPTADJUTOR = '3.0.9.533 patch 3',
       VERSAOSISTEMA.DATA   = '06.05.2019',
	    VERSAO = '3.0.9.533'
...
begin
    ...
    begin
        ...
        begin
           cst = (select FIRST 1 prd.prd_cstpiscofins from prd0000 prd where prd.prd_refer = new.prd_refer and prd.emp_codigo = new.emp_codigo);
        end
        else
           cst = (select FIRST 1 prd.prd_cstpiscofins from prd0000 prd where prd.prd_refer = new.prd_refer);
        new.cst_pis = cst;
        new.cst_cofins = cst;
    end
    else
...
ALTER TABLE PRMT0001 ADD PMT_ORDEM_CODFORN VARCHAR(1);
ALTER TABLE PRMT0001 ADD PMT_ORDEM_DESCFORN VARCHAR(1);

comment ON COLUMN PRMT0001.PMT_ORDEM_CODFORN  IS 'IMPRIMIR ORDEM DE COMPRA COM O CODIGO DO PRODUTO NO FORNECEDOR';
comment ON COLUMN PRMT0001.PMT_ORDEM_DESCFORN IS 'IMPRIMIR ORDEM DE COMPRA COM A DESCRICAO DO PRODUTO NO FORNECEDOR';


UPDATE PRMT0001 SET PMT_ORDEM_CODFORN = 'N', PMT_ORDEM_DESCFORN = 'N'; 

COMMIT WORK;

SET TERM ^ ;

create or alter procedure PCD_FLUXO_CAIXA (
    DATA_INICIAL date,
    DATA_FINAL date,
