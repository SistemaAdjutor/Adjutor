...
SET VERSAOSISTEMA.SCRIPTADJUTOR = '3.0.9.571 patch 2',
    VERSAOSISTEMA.DATA   = '18.08.2022',
	VERSAO = '3.0.9.571'
 where VERSAOSISTEMA.CODIGO = 1;
commit work;

ALTER TABLE PRMT0001 ADD PMT_ENF_CHK_INDUSTRIALIZACAO VARCHAR(1) DEFAULT 'N';
COMMENT ON COLUMN PRMT0001.PMT_ENF_CHK_INDUSTRIALIZACAO  is 'Marca Automaticamente a Caixa de Checagem de Industrialização na Entrada de Nota Fiscal';

ALTER TABLE PRMT0001 ADD PMT_NF_ESPECIE VARCHAR(60);
COMMENT ON COLUMN PRMT0001.PMT_NF_ESPECIE  is 'Alimenta Automaticamente o Campo Espécie no Faturamento da Nota Fiscal';
ALTER TABLE PRMT0001 ALTER PMT_NF_MARCA TYPE VARCHAR(60); 
ALTER TABLE NF0001 ALTER NF_ESPECIE TYPE VARCHAR(60); 
ALTER TABLE NF0001 ALTER NF_MARCA TYPE VARCHAR(60); 

ALTER TABLE ENF0001 ADD ENF_PESO_TOTAL NUMERIC(18,5);
...
declare variable PMT_CALCULARPV integer; /* - tipo de cÃ¡lculo do preÃ§o de venda */
declare variable PMT_NAO_AUTO_PVENDA varchar(1); /* nÃ£o atualiza preÃ§o de venda */
...
FOR 
    ...
    WHERE EXISTS 
           ...
           -- INNER JOIN ENF_IT01 E1 ON (EF.ENF_NOTANUMBER = E1.ENF_IT_NOTANUMBER)
            ...
            WHERE PR.PRD_REFER = E1.PRD_REFER  AND PR.prd_status='A' AND EF.emp_codigo = :emp_codigo
  ...
  --     (Select 1 from ftc_it01 ftc where ftc.prd_refer = pr.prd_refer and ftc.emp_codigo= :emp_codigo)
    ...
    BEGIN
      ...
      --  INNER JOIN ENF_IT01 E1 ON (EF.ENF_NOTANUMBER = E1.ENF_IT_NOTANUMBER)
        ...
        AND e1.enf_qtde > 0
        ORDER BY    EF.ENF_EMISSAO,
        EF.enf_registro DESC
      ...
      begin
        ...
        begin
            ...
            if ((:PRD_PMEDIO > 0 ) and (:PMT_CALCULARPV = 2)) then --custo de mÃ©dio
...
GRANT SELECT ON SHAREDB TO PROCEDURE ATUALIZACAO_CUSTOS;
GRANT SELECT ON PRMT0001 TO PROCEDURE ATUALIZACAO_CUSTOS;
...
GRANT SELECT ON OPE0000 TO PROCEDURE ATUALIZACAO_CUSTOS;
