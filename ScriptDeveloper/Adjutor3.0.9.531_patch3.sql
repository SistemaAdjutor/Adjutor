...
SET VERSAOSISTEMA.SCRIPTADJUTOR = '3.0.9.531 patch 3',
...
ALTER TABLE BAN0000 ADD BAN_DIASCRED_CARTAO   INTEGER;
COMMENT ON COLUMN BAN0000.BAN_DIASCRED_COBRANCA IS 'DIAS PARA CREDITO DO BOLETO DE COBRANCA'; 
COMMENT ON COLUMN BAN0000.BAN_DIASCRED_CARTAO IS 'DIAS PARA CREDITO DO CARTAO CREDITO';
      ...
      /* RECEBIDO PELO CONTA BANCO ANTES DO PERIODO E DEPOIS DA CONCILIACAO */ 
        ...
        begin
           ...
           WHERE BAN_CODIGO = :BAN_CODIGO
            ...
            LEFT JOIN BAN0000 B  ON (FT.BAN_CODIGO = B.BAN_CODIGO AND (FT.FPC_TIPODOC = 'BL' or FPG_DESCRICAO = 'BOLETO' ) AND (B.EMP_CODIGO = FT.EMP_CODIGO OR :BANCO_EXCLUSIVO='C'))
            LEFT JOIN BAN0000 Bc ON (FT.BAN_CODIGO = Bc.BAN_CODIGO AND FPG_DESCRICAO IN ('CARTÃO DE CRÉDITO', 'CARTAO DE CREDITO','CARTAO CREDITO')) 
                                AND (Bc.EMP_CODIGO = FT.EMP_CODIGO OR :BANCO_EXCLUSIVO='C' )                                    
            where FPC_EXCLUSAO = 'N'
            and (FPC_VENCTO + COALESCE(COALESCE(B.BAN_DIASCRED_COBRANCA,BC.BAN_DIASCRED_CARTAO),0)) BETWEEN :DATA_CONCILIACAO+1 AND  :DATA_INICIAL-1
    ...
    begin
     ...
     begin
        ...
        LEFT JOIN BAN0000 Bc ON (FT.BAN_CODIGO = Bc.BAN_CODIGO AND FPG_DESCRICAO IN ('CARTÃO DE CRÉDITO', 'CARTAO DE CREDITO','CARTAO CREDITO')) 
                            AND (Bc.EMP_CODIGO = FT.EMP_CODIGO OR :BANCO_EXCLUSIVO='C' )        
        where FPC_EXCLUSAO = 'N'
        and (FPC_VENCTO + COALESCE(COALESCE(B.BAN_DIASCRED_COBRANCA,BC.BAN_DIASCRED_CARTAO),0)) = :DATA_PROCESSO
        and (FT.EMP_CODIGO = :CODIGO_EMPRESA or :CODIGO_EMPRESA = '999')
        group by  (FPC_VENCTO + COALESCE(COALESCE(B.BAN_DIASCRED_COBRANCA,BC.BAN_DIASCRED_CARTAO),0))
