...
SET VERSAOSISTEMA.SCRIPTADJUTOR = '3.0.9.540 patch 12',
...
returns (
	...
	VALOR_RECEBIDO_ANTECIPADO numeric(18,2),
   ...
   begin
    for
     ...
     begin   
      ...
      /* xunxo - pegar as contas previsĂŁo a receber e pagar que nĂŁo foram colocadas o banco*/
            /* RECEBIDO */
   ...
   while (DATA_PROCESSO <= DATA_FINAL) do
    begin
     ...
     begin
        ...
        --and (fpc_pagto is null or (fpc_pagto = (FPC_VENCTO + COALESCE(BAN_DIASCRED_COBRANCA,0)))  )
        and (FT.EMP_CODIGO = :CODIGO_EMPRESA or :CODIGO_EMPRESA = '999')
        group by  (FPC_VENCTO + COALESCE(BAN_DIASCRED_COBRANCA,0))
        INTO :VALOR_PREVISAO_RECEBER;
		
		-- ENTRADA EM DATA ANETIOR
		Select COALESCE(sum(FPC_VLPARC),0)
        from FAT_PC01 FT
        LEFT JOIN FORMA_PAGAMENTO FP ON (FP.FPG_REGISTRO = FT.FPG_REGISTRO )
        LEFT JOIN BAN0000 B ON (FT.BAN_CODIGO = B.BAN_CODIGO AND (FT.FPC_TIPODOC = 'BL' OR FPG_DESCRICAO IN ('CARTÃO DE CRÉDITO', 'CARTAO DE CREDITO','CARTAO CREDITO','BOLETO'))
        ...
        where FPC_EXCLUSAO = 'N'
        and (FPC_VENCTO + COALESCE(BAN_DIASCRED_COBRANCA,0)) = :DATA_PROCESSO
        AND  (FPC_DESCONTADO = 'S' AND FPC_DATADESCTO < :DATA_PROCESSO )
        --and (fpc_pagto is null or (fpc_pagto = (FPC_VENCTO + COALESCE(BAN_DIASCRED_COBRANCA,0)))  )
        and (FT.EMP_CODIGO = :CODIGO_EMPRESA or :CODIGO_EMPRESA = '999')
        group by  (FPC_VENCTO + COALESCE(BAN_DIASCRED_COBRANCA,0))       
		INTO :VALOR_RECEBIDO_ANTECIPADO;
		
		VALOR_PREVISAO_RECEBER = VALOR_PREVISAO_RECEBER - VALOR_RECEBIDO_ANTECIPADO;
...
UPDATE NF0001 NF
SET NF_TIPONOTA = (SELECT FIRST 1 PRD_PRODSERV FROM NF_IT01 IT 
                     JOIN  PRD0000 PR ON (PR.PRD_CODIGO = IT.PRD_CODIGO ) 
                     WHERE NF.NF_NOTANUMBER = IT.NF_IT_NOTANUMER AND NF.EMP_CODIGO = IT.EMP_CODIGO
                     AND PRD_PRODSERV IS NOT NULL)
WHERE NF_TIPONOTA = '' OR NF_TIPONOTA IS NULL   ;
