...
var
  FormEnviaCelula: TFormEnviaCelula;
  wVct_tup:Double;

implementation

uses Uteis, Osv0001, CEL0008, Men0001, DmProdu, DataCad, iniciodb;

{$R *.DFM}

procedure TFormEnviaCelula.MudaCorCampos(Sender: tObject);
begin
...
procedure TFormEnviaCelula.HabilitaBotoes;
begin
...
procedure TFormEnviaCelula.DesabilitaBotoes;
begin
...
procedure TFormEnviaCelula.FormShow(Sender: tObject);
begin
    left := 25;
    Top  := 65;
    ...
    EditOsv_codigo.Text       := FormOrdServCentro.CdsOPOSV_CODIGO.AsString;
    EditPrd_refer.Text        := FormOrdServCentro.CdsOPPRD_REFER.AsString;
    EditPrdDescri.Text        := FormOrdServCentro.CdsOPPRD_DESCRI.AsString;
    CurrTupProd.Value         := FormOrdServCentro.CdsOPFTC_TUP.AsCurrency;
    CurrQtdeTotalProduz.Value := FormOrdServCentro.CdsOPOSV_QTDE.AsCurrency;
    ...
    try
      ...
      DmProducao.CdsFatorCustos.CommandText := SQLDEF('PRODUTOS','Select F1.* from VCT0000 F1 ','where F1.VCT_CLASSE = '''+FormOrdServCentro.SqlCdsParamPMT_EMPRESACLASSE.AsString+'''','F1.VCT_CLASSE','F1.');
...
procedure TFormEnviaCelula.Bit_ExcluirClick(Sender: tObject);
begin
...
procedure TFormEnviaCelula.Bit_CancelarClick(Sender: tObject);
begin
...
procedure TFormEnviaCelula.Bit_IncluirClick(Sender: tObject);
begin
    Screen.Cursor := crHourGlass;
    try
      ...
      DmProducao.CdsMovCelCEM_PRECOUNIT.AsCurrency := FormOrdServCentro.CdsOPOSV_PRECOUNIT.AsCurrency;
      DmProducao.CdsMovCelOSV_EMISSAO.AsDateTime   := FormOrdServCentro.CdsOPOSV_EMISSAO.AsDateTime;
      DmProducao.CdsMovCelOSV_QTDE.AsCurrency      := FormOrdServCentro.CdsOPOSV_QTDE.AsCurrency;
      DmProducao.CdsMovCelPRD_REFER.AsString       := FormOrdServCentro.CdsOPPRD_REFER.AsString;
      DmProducao.CdsMovCelCEM_TUPTRAB.AsCurrency   := FormOrdServCentro.CdsOPFTC_TUP.AsCurrency;
...
procedure TFormEnviaCelula.Bit_GravarClick(Sender: tObject);
var
...
procedure TFormEnviaCelula.DBGrid1KeyDown(Sender: tObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFormEnviaCelula.Bit_SairClick(Sender: tObject);
begin
    close;
end;

procedure TFormEnviaCelula.FormCloseQuery(Sender: tObject;
...
procedure TFormEnviaCelula.FormClose(Sender: tObject;
...
procedure TFormEnviaCelula.DbeDataEntradaExit(Sender: tObject);
begin
    if not TestaDataStr(DbeDataEntrada.Text) then
       DbeDataEntrada.SetFocus;
end;

procedure TFormEnviaCelula.DbDt_ConclusaoExit(Sender: tObject);
begin
...
procedure TFormEnviaCelula.CProduzidaEnter(Sender: tObject);
begin
    CProduzida.SelectAll;
end;

procedure TFormEnviaCelula.CdefeitoEnter(Sender: tObject);
begin
    Cdefeito.SelectAll;
end;

procedure TFormEnviaCelula.CdTProduzidaEnter(Sender: tObject);
begin
    CdTProduzida.SelectAll;
end;

procedure TFormEnviaCelula.CdTProduzidaExit(Sender: tObject);
begin
    if not TestaDataStr(CdTProduzida.Text) then
       CdTProduzida.setfocus;
end;

procedure TFormEnviaCelula.CProduzidaChange(Sender: tObject);
begin
    DmProducao.CdsMovCel.Edit;
    Desabilitabotoes;
end;

procedure TFormEnviaCelula.ValidaProducao;
begin
...
procedure TFormEnviaCelula.DbeOsv_ValorPecaEnter(Sender: tObject);
begin
    DbeOsv_ValorPeca.SelectAll;
end;

procedure TFormEnviaCelula.DbeOsv_ValorDefeEnter(Sender: tObject);
begin
    DbeOsv_ValorDefe.SelectAll;
end;

procedure TFormEnviaCelula.BitExcluiEntregaClick(Sender: tObject);
begin
...
procedure TFormEnviaCelula.ImprimeFichaEntrega;
var
...
procedure TFormEnviaCelula.Bit_ImprimiClick(Sender: tObject);
begin
    ImprimeFichaEntrega;
end;

procedure TFormEnviaCelula.SqlCdsOPCelulasBeforeOpen(DataSet: TDataSet);
begin
    try
      SqlCdsExecuta.close;
...
procedure TFormEnviaCelula.SqlCdsOPCelulasAfterScroll(DataSet: TDataSet);
begin
...
procedure TFormEnviaCelula.GroupEntregaEnter(Sender: tObject);
begin
