unit Fat0000_etiquetas;

interface

uses
     Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs,
     StdCtrls, DB, Grids, DBGrids, Mask, DBCtrls, SqlExpr, ExtCtrls, BaseDBForm, Data.DBXFirebird,
     Data.FMTBCd, ACBrEnterTab, ACBrBase, ACBrCalculadora, RDprint, ppCtrls, ppParameter, ppDesignLayer,
     ppBands, ppBarCod, ppPrnabl, ppClass, ppCache, ppProd, ppReport, ppDB, ppComm, ppRelatv,
     ppDBPipe, Datasnap.DBClient, Vcl.Buttons;

type

  TFrmFat0000_etiquetas = class(TfrmBaseDB)
  ...
  private
         WSQL1, WSQL2,  WSELECIONA, WORDEM: STRING;
...
var
  FrmFat0000_etiquetas: TFrmFat0000_etiquetas;

implementation

{$R *.dfm}

uses datacad, uteis, iniciodb, RWFUNC;


procedure TFrmFat0000_etiquetas.BitEtiqFecharClick(Sender: TObject);
begin
     Close;
end;

procedure TFrmFat0000_etiquetas.BitImprimeEtiqClick(Sender: TObject);
begin
...
procedure TFrmFat0000_etiquetas.FormCreate(Sender: TObject);
begin
    inherited;
    SELF.WIDTH:= 550;
    SELF.HEIGHT:= 270;
...
procedure TFrmFat0000_etiquetas.Imprimir_Etiqueta_Sistema;
Var
  I,wIQtde     : Integer;
...
begin
   ...
   try
     ...
     try
       ...
       //
       wSql1      := 'SELECT T1.NF_NOTANUMBER,T1.NF_NUM_NFE,T1.NF_QTDE,T4.TRP_RAZAO,T2.CLI_RAZAO,T2.CLI_FANTASIA,T2.CLI_ENDERE,T2.CLI_BAIRRO,T2.CLI_CEP,T2.CLI_CIDADE,T2.CLI_UF,T2.CLI_FONE,T2.CLI_ENDENTR,';
       wSql2      := 'T2.CLI_CIDENTR,T2.CLI_CEPENTR,T2.CLI_UFENTR,T2.CLI_FONENTR,T2.CLI_BAIENTR,T2.CLI_CONTATO FROM NF0001 T1  ';
       wSqlAdi    := 'LEFT JOIN CLI0000 T2 ON (T2.CLI_CODIGO = T1.CLI_CODIGO) LEFT JOIN PED0000 T3 ON (T1.PED_CODIGO = T3.PED_CODIGO) LEFT JOIN TRP0000 T4 ON (T3.TRP_CODIGO = T4.TRP_CODIGO)';
       wSeleciona := 'WHERE T1.NF_NOTANUMBER BETWEEN '''+EditInicial.Text+''' AND '''+EditFinal.Text+'''';
       wOrdem     := 'T1.NF_NOTANUMBER';
       //
       SqlCdsEtiqueta.Close;
       SqlCdsEtiqueta.sql.text := SQLDEF('RECEBER',wSQL1+wSQL2+wSqlAdi,wSeleciona,wOrdem,'T1.');
       SqlCdsEtiqueta.Open;
       I := 1;
       ...
       while not SqlCdsEtiqueta.Eof do
        begin
           ...
           for I := 1 to wIQtde do
             begin
                if (Gravar_End_Entrega) then
                   begin
                      CdsEtiqueta.Append;
                      CdsEtiquetaCLI_RAZAO.AsString  := SqlCdsEtiquetaCLI_RAZAO.AsString;
                      CdsEtiquetaCLI_ENDE.AsString   := SqlCdsEtiquetaCLI_ENDENTR.AsString;
                      CdsEtiquetaCLI_BAIRRO.AsString := SqlCdsEtiquetaCLI_BAIENTR.AsString;
                      CdsEtiquetaCLI_CEP.AsString    := MascaraCep(SqlCdsEtiquetaCLI_CEPENTR.AsString);
                      CdsEtiquetaCLI_CIDADE.AsString := trim(SqlCdsEtiquetaCLI_CIDENTR.AsString)+' - '+SqlCdsEtiquetaCLI_UFENTR.AsString;
       ...
       if (rbModelo3.Checked) then
         begin
            ...
            plblTelefone.Caption := dbInicio.Empresa.FONE+' '+dbInicio.Empresa.FAX;
       ...
       if (rbModelo5.Checked) then
         begin
            ...
            ppLabel34.Caption := dbInicio.Empresa.FONE+' '+dbInicio.Empresa.FAX;
...
begin
    try
      ...
      wSql3 := 'P1.PED_OBSNOTA,P1.PED_FRETE,P1.PED_FRETE2,P1.TRP_CODIGO,P1.TRP_REDESP,O1.OPE_DESCRINATUREZA,O1.OPE_REDU_ICM,O1.OPE_NOTA_COMPLEMENTAR,R1.REP_NOME From NF0001 N1 ';
      wSql4 := 'JOIN OPE0000 O1 ON O1.ope_codigo = t1.ope_codigo JOIN PED0000 P1 ON P1.PED_CODIGO = t1.ped_codigo JOIN CLI0000 C1 ON C1.CLI_CODIGO = P1.CLI_CODIGO ';
      wSql5 := 'LEFT JOIN REP0000 R1 ON R1.REP_CODIGO = P1.REP_CODIGO LEFT JOIN TRP0000 T1 ON T1.trp_codigo = P1.trp_codigo LEFT JOIN TRP0000 T2 ON T2.trp_codigo = P1.trp_redesp ';
                   ...
                   begin
                       ...
                       for I := 1 to wNumeroEtiq do
                        begin
                            ...
                            DataCadastros.SqlTransportadora.sql.text := SQLDEF('TRANSPORTADORAS','Select T1.* from TRP0000 T1 ','where T1.TRP_CODIGO = '''+CdsImpNotaTRP_CODIGO.AsString+'''','T1.TRP_CODIGO','T1.');
                            DataCadastros.CdsTransportadora.Open;
                            if DataCadastros.CdsTransportadora.IsEmpty = false then
                               begin
                                   {TRANSPORTADORA}
                                   if (Copy(SqlCdsParamEtiqCET_TRANSPORT.AsString,7,1) = 'S') then
                                      if wRotulos = 'S' then
                                         RdPrintEtiq1.imp(wLinha+StrToInt(COPY(SqlCdsParamEtiqCET_TRANSPORT.AsString,1,3)),StrToInt(COPY(SqlCdsParamEtiqCET_TRANSPORT.AsString,4,3))+wColuna,'Transportadora: '+DataCadastros.CdsTransportadoraTRP_RAZAO.AsString)
                                      else
                                         RdPrintEtiq1.imp(wLinha+StrToInt(COPY(SqlCdsParamEtiqCET_TRANSPORT.AsString,1,3)),StrToInt(COPY(SqlCdsParamEtiqCET_TRANSPORT.AsString,4,3))+wColuna,DataCadastros.CdsTransportadoraTRP_RAZAO.AsString);
                                   (*
...
function TFrmFat0000_etiquetas.Gravar_End_Entrega: Boolean;
begin
