...
uses
  ...
  RxLookup, ComboBoxRw, JvExComCtrls, JvDateTimePicker, JvDBDateTimePicker,
...
type
  TFrmPedidoVelho = class(TForm)
    ...
    SqlCdsClienteCLI_INSC: TStringField;
    SqlCdsClienteCID_CIDADE: TStringField;
    SqlCdsClienteCID_ESTADO: TStringField;
    SqlCdsClienteCLI_CONSFINAL: TStringField;
    SqlCdsClientePCL_CODIGO: TStringField;
    SqlCdsClientePCL_NOME: TStringField;
    SqlCdsClienteCLI_FONE: TStringField;
    ...
    SqlQueryClienteCLI_CODIGO: TStringField;
    SqlQueryClienteCLI_RAZAO: TStringField;
    ...
    SqlCdsClienteREP_CODIGO: TStringField;
    SqlCdsClienteTRP_CODIGO: TStringField;
    ...
    SqlCdsClienteOPE_CODIGO: TStringField;
    SqlCdsTipoPedidoOPV_CALCULA_ST: TStringField;
    SqlCdsClienteCLI_REGIME_TRIBUTARIO: TStringField;
    ...
    SqlCdsClientePCX_CODIGO: TStringField;
    ...
    SqlCdsClienteCLI_TABPRECO: TStringField;
    ...
    SqlCdsClienteCLI_MODO_TRIB_ST: TStringField;
    SqlCdsClienteCNAE_CARGA_TRIB_MEDIA: TFMTBCdField;
    Label114: TLabel;
    ...
    procedure CbTransportadoraExit(Sender: tObject);
    ...
    procedure EdVendedorCodigoChange(Sender: tObject);
...
var
  FrmPedidoVelho: TFrmPedidoVelho;
implementation
uses
...
{ TFrmPedido }
procedure TFrmPedidoVelho.AdicionaItem;
...
procedure TFrmPedidoVelho.BuscaCliente(sCodigo: String);
var
   sQuery:String;
begin
   sQuery := 'select t1.cli_codigo, '+
             '       t1.cli_tabpreco, '+
             '       t1.cli_razao, '+
             '       t1.cli_cgc, '+
             '       t1.cli_insc, '+
   ...
   //Trata Campo passado
   sCodigo := PreenchezeroEsquerda(sCodigo,5);
   bMostraMsgAtraso := (sCodigo = '');
   SqlCdsCliente.Close;
   SqlCdsCliente.CommandText := SQLDEF('CLIENTES',sQuery,'where t1.cli_codigo = '+QuotedStr(sCodigo),'','t1.');
   SqlCdsCliente.Open;
   //Limpa Campos
   EdCnpj.Clear;
   ...
   EdClienteUF.Text := SqlCdsClienteCID_ESTADO.AsString;
   if (sCfopClienteCodigo = '') then
      sCfopClienteCodigo := SqlCdsClienteOPE_CODIGO.AsString;
   EdClienteConsumidorFinal.Clear;
   EdClientePrazo.Clear;
   EdClienteTelefone.Clear;
   //CbCliente.Text := '';
   //EdClienteCodigo.Clear;
   EdClienteCodigo.Text := SqlCdsClienteCLI_CODIGO.AsString;
   sCodCliente := EdClienteCodigo.Text ;
   pnlAtraso.Visible := False;
   // Se o SqlPedido = Empty então sei que é im insert
   if (SqlCdsPedido.IsEmpty) then
   begin
         EdPrazoCodigo.Clear;
         CbPrazo.Text := '';
   ...
   Else
   begin
         EdCnpj.Text := MascaraCNPJ_CPF(SqlCdsClienteCLI_CGC.AsString);
         if (Length(SqlCdsClienteCLI_CGC.AsString) = 14) then  //Pessoa Juridica
         begin
               LblCnpj.Caption := 'CNPJ:';
               LblIE.Caption := 'IE:';
         end
         ...
         EdIE.Text := SqlCdsClienteCLI_INSC.AsString;
         EdClienteCidade.Text := SqlCdsClienteCID_CIDADE.AsString;
         EdClienteUF.Text := SqlCdsClienteCID_ESTADO.AsString;
         EdClienteConsumidorFinal.Text := iif(SqlCdsClienteCLI_CONSFINAL.AsString = 'S','SIM','NÃO');
         EdClientePrazo.Text := SqlCdsClientePCL_NOME.AsString;
         EdClienteTelefone.Text := MascaraFone(SqlCdsClienteCLI_FONE.AsString);
         // Se o SqlPedido = Empty então sei que é im insert
         if (SqlCdsPedido.IsEmpty) then
         begin
               EdPrazoCodigo.Text := SqlCdsClientePCL_CODIGO.AsString;
               CbPrazo.TextoLocalizar := EdPrazoCodigo.Text;
               CbPrazo.Localizar := True;
               EdVendedorCodigo.Text := SqlCdsClienteREP_CODIGO.AsString;
               CbVendedor.TextoLocalizar := EdVendedorCodigo.Text;
               CbVendedor.Localizar := True;
               VerificaComissaoVendedor;
               EdTransportadoraCodigo.Text := SqlCdsClienteTRP_CODIGO.AsString;
               CbTransportadora.TextoLocalizar := EdTransportadoraCodigo.Text;
               CbTransportadora.Localizar := True;
               sCfopClienteCodigo := SqlCdsClienteOPE_CODIGO.AsString;
               CbCfop.EditValue := sCfopClienteCodigo;
               EdProjetoObraCodigo.Text := SqlCdsClientePCX_CODIGO.AsString;
...
procedure TFrmPedidoVelho.BuscaPedido(sPedidoCodigo:String);
var
   sQuery:String;
begin
   //Desabilita Edição
   ...
   begin
         ...
         //Prazo
         EdPrazoCodigo.Text := SqlCdsPedidoPCL_CODIGO.AsString;
         CbPrazo.TextoLocalizar := EdPrazoCodigo.Text;
         CbPrazo.Localizar := True;
         ...
         //Vendedor
         EdVendedorCodigo.Text := SqlCdsPedidoREP_CODIGO.AsString;
         CbVendedor.TextoLocalizar := EdVendedorCodigo.Text;
         CbVendedor.Localizar := True;
         ...
         //Transportadora
         EdTransportadoraCodigo.Text := SqlCdsPedidoTRP_CODIGO.AsString;
         CbTransportadora.TextoLocalizar := EdTransportadoraCodigo.Text;
         CbTransportadora.Localizar := True;
         //CFOP
         ...
         CbTipoFrete.ItemIndex := iif(SqlCdsPedidoPED_FRETE.AsString = '0',0,
                                  ...
                                  3)));
         //Projeto Obra
         EdProjetoObraCodigo.Text := SqlCdsPedidoPCX_CODIGO.AsString;
         if (EdProjetoObraCodigo.Text = '') then
            begin
               EdProjetoObraCodigo.Text := SqlCdsPedidoCENTRO_CUSTO_CLIENTE.AsString;
            end;
         ...
         CBFormaPagamento.TextoLocalizar := SqlCdsPedidoFPG_REGISTRO.AsString;
         CBFormaPagamento.Localizar := True;
...
procedure TFrmPedidoVelho.BuscaPedidoItem(sPedidoCodigo: String);
var
...
procedure TFrmPedidoVelho.CalculaIndiceDescto;
begin
...
procedure TFrmPedidoVelho.CalcutaTotalItens;
var
   rDescontoItem,
...
procedure TFrmPedidoVelho.FormKeyDown(Sender: tObject; var Key: Word;
  Shift: TShiftState);
begin
     if (Key = VK_F2) then
     begin
...
procedure TFrmPedidoVelho.HabilitaDesabilitaEdicao(bStatus: Boolean);
begin
   PageControl1.ActivePageIndex := 0;
   PageControl2.ActivePageIndex := 0;
   bEdicao := bStatus;
...
procedure TFrmPedidoVelho.VerificaSePodeEditar;
begin
end;
procedure TFrmPedidoVelho.CurDescontoNotaFiscalExit(Sender: tObject);
begin
   CalculaIndiceDescto;
end;
procedure TFrmPedidoVelho.CurDescontoNotaFiscalAdicionalExit(Sender: tObject);
begin
   CalculaIndiceDescto;
end;
procedure TFrmPedidoVelho.FormShow(Sender: tObject);
var
   sCondicao : String;
begin
   ...
   if (dbInicio.Empresa.bVisualizaClientes_P) and (dbInicio.Usuario.Codigo_REPRES <> '000') then
       sCondicao := ' WHERE REP_CODIGO ='''+dbInicio.Usuario.Codigo_REPRES+'''';
   CdsCliente.Close;
   CdsCliente.CommandText := SQLDEF('CLIENTES','SELECT T1.cli_codigo,T1.cli_razao FROM cli0000 T1 ',sCondicao,'','t1.')+ ' ORDER BY T1.cli_razao';
   CdsCliente.Open;
   //Combo
   CbCliente.CodigoEmpresa := StrToInt(dbInicio.Empresa.EMP_CODIGO);
   CbCliente.Compartilhar := 'CLIENTES';
   if (sCondicao <> '') then
      CbCliente.Condicao :=  'REP_CODIGO ='''+dbInicio.Usuario.Codigo_REPRES+''''; 
   CbCliente.CarregarCombo := True;
   CbPrazo.CarregarCombo := True;
   CbVendedor.CarregarCombo := True;
   CbTransportadora.CarregarCombo := True;
...
procedure TFrmPedidoVelho.BtnConsultarClick(Sender: tObject);
begin
      ...
      begin
         ...
         try
            ...
            begin
                 ...
                 BuscaCliente(SqlCdsPedidoCLI_CODIGO.AsString);
...
procedure TFrmPedidoVelho.CurFreteExit(Sender: tObject);
begin
   CalcutaTotalItens;
end;

procedure TFrmPedidoVelho.DbGradeItemPedidoDrawColumnCell(Sender: tObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
var
   Check: Integer;
...
procedure TFrmPedidoVelho.AlterarItem;
var lItb: tBookMark;
begin
...
procedure TFrmPedidoVelho.ExcluirItem;
begin
   if (not SqlCdsPedidoItem.IsEmpty) then
      begin
...
function TFrmPedidoVelho.GravaPedido: Boolean;
var
   sMsg: String;
begin
   PageControl1.ActivePageIndex := 0;
   ...
   if (EdProjetoObraCodigo.Text = '')and(FrmMenu.bCentroCustoObrigatorio) then
   begin
   ...
   begin
        ...
        if bTipoPedidoCalculaST = True then
         ...
         CbVendedor.CarregarCombo := False;
         CbVendedor.Condicao := '';
         CbVendedor.CarregarCombo := True;
...
Function TFrmPedidoVelho.GetTipoPedidoPadrao:integer;
var vQry: tSqlQuery;
begin
  ...
  try
    ...
    vQry.SQLConnection:=DataCadastros.SQLConnection1;
    vQry.sql.Text:=SQLDEF('TABELAS','Select OPV_CODIGO from OPV0000','Where OPV_TIPOPADRAO = ''S''','','');
    vQry.active := True; 
    //clipbrd.clipboard.astext := vQry.sql.Text;
...
procedure TFrmPedidoVelho.IncluirPedido;
begin
   PageControl1.ActivePageIndex := 0;
   PageControl2.ActivePageIndex := 0;
   ...
   BuscaCliente('');
   BuscaPedidoItem('');
   //pega somente vendedores ativos
   CbVendedor.CarregarCombo := False;
   CbVendedor.Condicao := 'REP_SITUACAO = ''A''';
   CbVendedor.CarregarCombo := True;
...
procedure TFrmPedidoVelho.BtnNovoClick(Sender: tObject);
begin
   if (LiberaSairPedido) then
      IncluirPedido;
end;

procedure TFrmPedidoVelho.RemoverInfoNumeracaoPedido;
begin
...
procedure TFrmPedidoVelho.CancelarPedido;
begin
   if (bInclusao) then
   begin
...
procedure TFrmPedidoVelho.BtnCancelarClick(Sender: tObject);
begin
   CancelarPedido;
end;

procedure TFrmPedidoVelho.BtnGravarClick(Sender: tObject);
...
procedure TFrmPedidoVelho.BuscaTipoPedido(sTipoCodigo: Integer);
var
   sQuery:WideString;
begin
   ...
   if (sTipoCodigo > 0) then
      begin
         sQuery := 'SELECT t1.* FROM OPV0000 t1';
         SqlCdsTipoPedido.CommandText := SQLDEF('TABELAS',sQuery,'WHERE T1.OPV_CODIGO ='+QuotedStr(IntToStr(sTipoCodigo)),'','t1.')+ ' ORDER BY t1.OPV_DESCRICAO';
...
procedure TFrmPedidoVelho.BtnAlterarClick(Sender: tObject);
begin
   //Verifica se o Pedido já nao foi faturado totalmente
   if (SqlCdsPedidoPED_SITUACAO.AsString = 'C') then
      begin
...
procedure TFrmPedidoVelho.BtnExcluirClick(Sender: tObject);
begin
   ...
   if (uteis.confirmacao ( ('Deseja Cancelar o Pedido '''+EdPedidoNumero.text+''' do Cliente '''+SqlCdsClienteCLI_RAZAO.AsString+''' ?'))=mrYes) then
      ExcluirPedido;
end;

procedure TFrmPedidoVelho.DeletarItem;
begin
     SqlCdsPedidoItem.DISABLECONTROLS;
     TRY
                     ...
                     if (FrmMenu.wAtualiza_Estoque = 'P') then
                        begin
                           KardexLancamento(SqlCdsPedidoItemPRF_REGISTRO.AsString,
                                            ...
                                            SqlCdsClienteCLI_CODIGO.AsString+'-'+SqlCdsClienteCLI_RAZAO.AsString,
...
procedure TFrmPedidoVelho.Excluir1Click(Sender: tObject);
begin
   ExcluirItem;
end;
procedure TFrmPedidoVelho.Alterar1Click(Sender: tObject);
begin
   AlterarItem;
end;
procedure TFrmPedidoVelho.DbGradeItemPedidoDblClick(Sender: tObject);
begin
   AlterarItem;
end;
procedure TFrmPedidoVelho.BuscaVendaItemHistorico;
var
   sQuery:String;
begin
...
procedure TFrmPedidoVelho.TabHostoricoVendaShow(Sender: tObject);
begin
   BuscaVendaItemHistorico;
end;

procedure TFrmPedidoVelho.BtnEstoqueClick(Sender: tObject);
begin
   try
...
procedure TFrmPedidoVelho.ImprimePedido(bVisualiza: Boolean; sTipo:String);
...
procedure TFrmPedidoVelho.BtnImprimirClick(Sender: tObject);
begin
   if (SqlCdsTipoPedidoOPV_ORCAMENTO.AsString = 'S') then
...
procedure TFrmPedidoVelho.BtnEmailClick(Sender: tObject);
begin
   ImprimePedido(False,'');
end;

procedure TFrmPedidoVelho.FormClose(Sender: tObject; var Action: TCloseAction);
begin
   ...
   CbPrazo.CarregarCombo := False;
   CbVendedor.CarregarCombo := False;
   CbTransportadora.CarregarCombo := False;
   CbProjetoObra.CarregarCombo := False;
   CBFormaPagamento.CarregarCombo := False;

end;
procedure TFrmPedidoVelho.BtnRelatoriosClick(Sender: tObject);
begin
   try
...
procedure TFrmPedidoVelho.CdSPedidoDIAfterInsert(DataSet: TDataSet);
begin
...
procedure TFrmPedidoVelho.CdSPedidoDIAfterDelete(DataSet: TDataSet);
begin
  try
...
procedure TFrmPedidoVelho.CdSPedidoDIAfterPost(DataSet: TDataSet);
begin
...
procedure TFrmPedidoVelho.CdSPedidoDIAdicaoAfterDelete(DataSet: TDataSet);
begin
  try
...
procedure TFrmPedidoVelho.CdSPedidoDIAdicaoAfterInsert(DataSet: TDataSet);
begin
...
procedure TFrmPedidoVelho.CdSPedidoDIAdicaoAfterPost(DataSet: TDataSet);
begin
...
procedure TFrmPedidoVelho.DsPedidoItemDataChange(Sender: tObject;
  Field: TField);
begin
  CdSPedidoDI.Close;
...
procedure TFrmPedidoVelho.DSPedidoDIDataChange(Sender: tObject; Field: TField);
begin
  CdSPedidoDIAdicao.Close;
  if ((not CdSPedidoDI.IsEmpty) and (CdSPedidoDIPID_REGISTRO.AsString <> '')) then
    begin
...
procedure TFrmPedidoVelho.BuscaItensNota;
begin
   try
...
procedure TFrmPedidoVelho.BuscaNotas;
begin
...
procedure TFrmPedidoVelho.TabNotasFiscaisClienteShow(Sender: tObject);
begin
   BuscaNotas;
end;

procedure TFrmPedidoVelho.LayOutPesqClienteVenda;
begin
     Msk_busca.text      := '';
     Msk_busca.EditMask  := '';
...
procedure TFrmPedidoVelho.RadFantasiaClick(Sender: tObject);
begin
   LayOutPesqClienteVenda;
end;

procedure TFrmPedidoVelho.BuscaClienteVenda;
begin
     DataCadastros.CdsClientes.Close;
     if RadRazao.Checked  then
...
procedure TFrmPedidoVelho.BitPesquisaClick(Sender: tObject);
begin
   BuscaClienteVenda;
end;

procedure TFrmPedidoVelho.TabClienteVendasShow(Sender: tObject);
begin
   sCodCliente := EdClienteCodigo.Text ;
   if sCodCliente <> '' then
   begin
...
procedure TFrmPedidoVelho.BuscaprodutosCliente;
...
procedure TFrmPedidoVelho.AbreGraficoVendas;
begin
...
procedure TFrmPedidoVelho.SQLClienteProdCalcFields(DataSet: TDataSet);
begin
   SQLClienteProdCC_TOTAL.AsFloat := SQLClienteProdNF_QTDE.AsFloat*SQLClienteProdNF_PRECO.AsFloat;
end;
procedure TFrmPedidoVelho.SQLReservaProdAfterScroll(DataSet: TDataSet);
begin
// buscar os pedidos refente a cada produto que esta na reserva
   BuscapedidosReservas(SQLReservaProdPRD_REFER.AsString);
end;
procedure TFrmPedidoVelho.BitCancelaReservaProdClick(Sender: tObject);
begin
  EditProdReserva.Text := '';
  BuscaprodutosReservas;
end;
procedure TFrmPedidoVelho.BitPesqReservaProdClick(Sender: tObject);
var
...
procedure TFrmPedidoVelho.TabVendasFaturarShow(Sender: tObject);
begin
   BuscaprodutosReservas;
end;
function TFrmPedidoVelho.BuscapedidosReservas(
...
procedure TFrmPedidoVelho.BuscaprodutosReservas;
var
wsql1,wsql2,wsql3 :string;
begin
  wsql1 := '';
...
procedure TFrmPedidoVelho.Bit_SairClick(Sender: tObject);
begin
   if (SqlCdsPedido.IsEmpty) then
      Close
   else
...
procedure TFrmPedidoVelho.spClienteClick(Sender: tObject);
begin
   try {Cria e chama formulário de Lista}
      FormClienteGrid := TFormClienteGrid.Create(Application);
      FormClienteGrid.Tag := IIF(GBInformacoesCliente.Enabled,2,3);
...
procedure TFrmPedidoVelho.HabilitaDesabilitaDescontoNota(bStatus: Boolean);
begin
   if (bStatus) then
      begin
...
procedure TFrmPedidoVelho.CbDescontoNFClick(Sender: tObject);
begin
   HabilitaDesabilitaDescontoNota(CbDescontoNF.Checked);
end;

procedure TFrmPedidoVelho.EdClienteCodigoExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.CbClienteExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.CbClienteChange(Sender: tObject);
begin
     EdClienteCodigo.Text := CbCliente.CodigoLista;
end;

procedure TFrmPedidoVelho.EdPrazoCodigoExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.CbPrazoChange(Sender: tObject);
begin
   EdPrazoCodigo.Text := CbPrazo.CodigoLista;
end;
procedure TFrmPedidoVelho.CbPrazoExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.EdVendedorCodigoExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.CbVendedorChange(Sender: tObject);
begin
   EdVendedorCodigo.Text := CbVendedor.CodigoLista;
   VerificaComissaoVendedor;
end;

procedure TFrmPedidoVelho.EdTransportadoraCodigoExit(Sender: tObject);
begin
   ...
   if (EdTransportadoraCodigo.Text = '000') then
      ...
      begin
         CbTransportadora.TextoLocalizar := EdTransportadoraCodigo.Text;
         CbTransportadora.Localizar := True;
         if (CbTransportadora.Localizado) then
            begin
            end
         else
            begin
               ...
               EdTransportadoraCodigo.SelectAll;
            end;
      end;
end;
procedure TFrmPedidoVelho.CbTransportadoraChange(Sender: tObject);
begin
   EdTransportadoraCodigo.Text := CbTransportadora.CodigoLista;
end;
procedure TFrmPedidoVelho.CbTransportadoraExit(Sender: tObject);
begin
  if (CbTransportadora.CodigoLista = '') or (CbTransportadora.Text = '') then
      begin
         CbTransportadora.Text := '';
         EdTransportadoraCodigo.Text := '';
      end
...
procedure TFrmPedidoVelho.CbProjetoObraChange(Sender: tObject);
begin
   EdProjetoObraCodigo.Text := CbProjetoObra.CodigoLista;
end;
procedure TFrmPedidoVelho.CbProjetoObraExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.EdProjetoObraCodigoExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.CbVendedorExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.Adicionar1Click(Sender: tObject);
begin
   AdicionaItem;
end;

procedure TFrmPedidoVelho.SpeedTipoClick(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.SetCfopOpvTipo( tipo:string );
var vDentroForaEstado: string;
begin
   CdsCFOP.Close;
   if tipo<>'' then
   begin
      if SqlCdsClienteCID_ESTADO.AsString=dbInicio.Empresa.UF then
         vDentroForaEstado:='D'
      Else
         vDentroForaEstado:='F';
...
procedure TFrmPedidoVelho.GravaMediaComissao;
begin
...
procedure TFrmPedidoVelho.MovimentaEstoque(bStatus: Boolean);
begin
   if (bStatus) then
      begin
...
procedure TFrmPedidoVelho.ExcluirPedido;
var
   obs:String;
begin
...
procedure TFrmPedidoVelho.PedidoNormal1Click(Sender: tObject);
begin
   ImprimePedido(true,'');
end;

procedure TFrmPedidoVelho.PedidosemValores1Click(Sender: tObject);
begin
   ImprimePedido(true,'SV');
end;

procedure TFrmPedidoVelho.LerImagem ;
begin
  DataCadastros.sqlUpdate.Close;
...
procedure TFrmPedidoVelho.SpeedButton1Click(Sender: tObject);
begin
     AdicionaItem;
end;

procedure TFrmPedidoVelho.EdPedidoNumeroExit(Sender: tObject);
begin
...
Function TFrmPedidoVelho.NumeracaoRepetida: Boolean;
var vId : Integer;
    vSql : string ;
begin
...
procedure TFrmPedidoVelho.CdSPedidoDIBeforeEdit(DataSet: TDataSet);
begin
   if (CdSPedidoDI.IsEmpty) then
      begin
         CdSPedidoDI.Insert;
...
procedure TFrmPedidoVelho.EdNumeroPedidoClienteExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.EdNumeroPedidoClienteEnter(Sender: tObject);
begin
   sPedidoClienteAnterior := EdNumeroPedidoCliente.Text;
end;

procedure TFrmPedidoVelho.Oramento1Click(Sender: tObject);
begin
   ImprimePedido(true,'OR');
end;
procedure TFrmPedidoVelho.EdPedidoTipoChange(Sender: tObject);
begin
   Caption := EdPedidoTipo.Text;
end;
procedure TFrmPedidoVelho.Image1Click(Sender: tObject);
begin
   if (SqlCdsTipoPedidoOPV_ORCAMENTO.AsString = 'S') then
      begin
         EdClienteCodigo.Clear;
...
procedure TFrmPedidoVelho.Image2Click(Sender: tObject);
begin
   EdOrsCliente.Clear;
   GBInformacoesOrcamento.Visible := False;
   GBInformacoesCliente.Visible := True;
...
procedure TFrmPedidoVelho.SpeedButton2Click(Sender: tObject);
begin
   try
...
procedure TFrmPedidoVelho.SpeedButton3Click(Sender: tObject);
begin
   try
      FrmCadastroObservacao := TFrmCadastroObservacao.Create(Application);
...
procedure TFrmPedidoVelho.EdOrsDiaEntregaExit(Sender: tObject);
begin
   DteEntrega.Date := Verificadiadata(DteEntrada.Date,EdOrsDiaEntrega.AsInteger);
end;
procedure TFrmPedidoVelho.DteEntregaExit(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.DteEntradaExit(Sender: tObject);
begin
   if (DteEntrada.Text = '  /  /    ') then
      DteEntrada.Date := Now;
   DteEntrega.Date := Verificadiadata(DteEntrada.Date,EdOrsDiaEntrega.AsInteger);
end;
procedure TFrmPedidoVelho.EspelhodaNotaFiscal1Click(Sender: tObject);
begin
   ImprimePedido(true,'ESPELHO');
end;

procedure TFrmPedidoVelho.VerificaComissaoVendedor;
begin
...
procedure TFrmPedidoVelho.EdVendedorCodigoChange(Sender: tObject);
begin
   CurComissaoVendedor.Clear;
end;
procedure TFrmPedidoVelho.MenuItem1Click(Sender: tObject);
begin
   ImprimePedido(False,'');
end;
procedure TFrmPedidoVelho.MenuItem2Click(Sender: tObject);
begin
   ImprimePedido(False,'SV');
end;
procedure TFrmPedidoVelho.MenuItem3Click(Sender: tObject);
begin
   ImprimePedido(False,'OR');
end;
procedure TFrmPedidoVelho.MenuItem4Click(Sender: tObject);
begin
   ImprimePedido(False,'ESPELHO');
end;
procedure TFrmPedidoVelho.JvArrowButton1Click(Sender: tObject);
begin
   if (SqlCdsTipoPedidoOPV_ORCAMENTO.AsString = 'S') then
      ImprimePedido(False,'OR')
   else
...
function TFrmPedidoVelho.LiberaSairPedido: Boolean;
begin
   if (SqlCdsPedido.IsEmpty) then
      Result := True
   else
...
procedure TFrmPedidoVelho.FormCloseQuery(Sender: tObject;
...
procedure TFrmPedidoVelho.AlterarComisso1Click(Sender: tObject);
begin
  AlterarComissao;
end;
procedure TFrmPedidoVelho.AlterarComissao;
   var
      sComissao:String;
      rComissao:Currency;
begin
...
procedure TFrmPedidoVelho.SpeedButton4Click(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.Reajuste;
begin
   if (SqlCdsPedido.IsEmpty) then
      begin
...
procedure TFrmPedidoVelho.DupicarCopiar(sTipo:String);
begin
...
procedure TFrmPedidoVelho.BtnDuplicarClick(Sender: tObject);
Var
...
begin
    ...
    if (EdPedidoNumero.Text <> '') then
       begin
             ...
             try
               CdsCliente.Locate('CLI_CODIGO',EdClienteCodigo.Text,[]);
               FormPedidoDuplicar := TFormPedidoDuplicar.Create(Application);
               FormPedidoDuplicar.Edt_Cod_Pedido.Text  := EdPedidoNumero.Text;
               FormPedidoDuplicar.Edt_Nome_Pedido.Text := CdsClienteCLI_RAZAO.AsString;
               FormPedidoDuplicar.Edt_Cod_Cliente.Text  := CdsClienteCLI_CODIGO.AsString;
               FormPedidoDuplicar.edClienteDestino.Text := CdsClienteCLI_CODIGO.AsString;
...
procedure TFrmPedidoVelho.DBEdit28Change(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.btnEditarEndEntregaClick(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.btnGravarEndEntregaClick(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.PageControl1Change(Sender: tObject);
begin
     sCodCliente := EdClienteCodigo.Text;
end;

procedure TFrmPedidoVelho.PedidoComImagem1Click(Sender: tObject);
begin
   ImprimePedido(true,'IMG');
end;
procedure TFrmPedidoVelho.PedidoComImagem2Click(Sender: tObject);
begin
   ImprimePedido(False,'IMG');
end;
procedure TFrmPedidoVelho.PedidoComImagemCentro1Click(Sender: tObject);
begin
   ImprimePedido(true,'IMGC');
end;
procedure TFrmPedidoVelho.PedidoComImagemGrande1Click(Sender: tObject);
begin
   ImprimePedido(true,'IMGG');
end;

procedure TFrmPedidoVelho.OramentoImagem1Click(Sender: tObject);
begin
   ImprimePedido(True,'IMG');
end;

procedure TFrmPedidoVelho.OramentoImagemGrande1Click(Sender: tObject);
begin
   ImprimePedido(True,'IMGG');
end;
procedure TFrmPedidoVelho.OramentoImagem2Click(Sender: tObject);
begin
   ImprimePedido(False,'IMG');
end;
procedure TFrmPedidoVelho.OramentoImagemGrande2Click(Sender: tObject);
begin
    ImprimePedido(False,'IMGG');
end;

procedure TFrmPedidoVelho.Romaneio2Click(Sender: tObject);
begin
   ImprimePedido(True,'ROM');
end;
procedure TFrmPedidoVelho.Romaneio1Click(Sender: tObject);
begin
   ImprimePedido(False,'ROM');
end;

procedure TFrmPedidoVelho.PedidoComLinha1Click(Sender: tObject);
begin
   ImprimePedido(False,'LINHA');
end;

procedure TFrmPedidoVelho.PedidoComLinhadoProduto1Click(Sender: tObject);
begin
   ImprimePedido(True,'LINHA');
end;

procedure TFrmPedidoVelho.OramentoComLinhadoProduto1Click(Sender: tObject);
begin
    ImprimePedido(True,'ORCAMENTOLINHA');
end;

procedure TFrmPedidoVelho.OramentoComLinhadoProduto2Click(Sender: tObject);
begin
    ImprimePedido(False,'ORCAMENTOLINHA');
end;

procedure TFrmPedidoVelho.CbCfopPropertiesChange(Sender: tObject);
begin
   CbCfop.Hint :=  CbCfop.Text;
end;
procedure TFrmPedidoVelho.RecalculaSTItem;
var
   rRetorno,
   wUfAliqMVA,
   wIcmsAliq,
              ...
              if (bTipoPedidoCalculaST)and(SqlCdsPedidoItemPRD_REFER.AsString <> '')and(sCfopClienteCodigo <> '')and(EdClienteUF.Text <> '') then
                 begin
                    ...
                    //   begin
                          ...
                          if (bLocalizadoRegra)and(SqlCdsClienteCLI_CONSFINAL.AsString <> 'S') then
                            begin
                                ...
                                if (SqlCdsClienteCLI_CONSFINAL.AsString <> 'S') then
                                   begin
                                      //REGIME TRIBUTÁRIO DO SIMPLES TEM SEU MVA PROPRIO S=SIMPLES P = PRESUMIDO R = REAL
                                      if (SqlCdsClienteCLI_REGIME_TRIBUTARIO.AsString = 'S') then
                                        begin
                          ...
                          if (wTemSubs = 'S')and
                             (wIcmsAliq > 0) then
                             begin
                                if (SqlCdsClienteCLI_MODO_TRIB_ST.AsString = 'CNAE')and(SqlCdsClienteCNAE_CARGA_TRIB_MEDIA.AsFloat > 0) then
                                  begin
                                     if (SqlCdsPedidoItemPRF_IPIALIQ.AsFloat > 0) then
                                        rPrecoTotal := rPrecoTotal + (SqlCdsPedidoItemTOTAL.AsFloat* (SqlCdsPedidoItemPRF_IPIALIQ.AsFloat/100));
                                     rRetorno := rPrecoTotal * (SqlCdsClienteCNAE_CARGA_TRIB_MEDIA.AsFloat /100);
                                  end
                                else
                                  begin
                                    {UF destino}
...
procedure TFrmPedidoVelho.CbCfopExit(Sender: tObject);
begin
   if (CbCfop.EditValue <> null) then
   begin
...
procedure TFrmPedidoVelho.EnviarPedidoProducao;
var
...
procedure TFrmPedidoVelho.CBFormaPagamentoExit(Sender: tObject);
begin
  if (bAdicionadoForma = False)and(CBFormaPagamento.Text <> '') then
    begin
      if (uteis.confirmacao  ( 'Gostaria de adicionar a forma de pagamento na Observação do Pedido ?')=mrYes) then
        begin
           bAdicionadoForma := True;
           MemoObservacaoPedido.Lines.Add('Forma de Pagamento: '+CBFormaPagamento.Text);
        end;
    end;
end;

procedure TFrmPedidoVelho.btnRemoverIndustrializacaoClick(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.ConsultaRetornoIndustrializacao;
begin
  //
...
procedure TFrmPedidoVelho.btnVincularIndustrializacaoClick(Sender: tObject);
begin
  if (SqlCdsPedidoItem.IsEmpty) then
    begin
...
procedure TFrmPedidoVelho.RetornoIndustrializacao;
begin
  if (BtnGravar.Enabled) then
      begin
         BtnGravarClick(Self);
...
procedure TFrmPedidoVelho.PedidoSemItensdopKIT1Click(Sender: tObject);
begin
...
procedure TFrmPedidoVelho.EdClienteCodigoChange(Sender: tObject);
begin
  CdsClienteContato.Close;
  if (EdClienteCodigo.Text <> '') then
  begin
      CdsClienteContato.CommandText := 'select * from CLI_CONTATO T1 where CLI_CODIGO = '+QuotedStr(EdClienteCodigo.Text)+' ORDER BY T1.cc_nome';
      CdsClienteContato.Open;
  end;
end;

procedure TFrmPedidoVelho.CdsClienteContatoAfterPost(DataSet: TDataSet);
begin
  CdsClienteContato.ApplyUpdates(0);
  CdsClienteContato.Refresh;
end;

procedure TFrmPedidoVelho.CdsClienteContatoBeforePost(DataSet: TDataSet);
begin
  if (EdClienteCodigo.Text <> '') then
    CdsClienteContatoCLI_CODIGO.AsString := EdClienteCodigo.Text;
end;

procedure TFrmPedidoVelho.OrcamentoImagemSomenteTotal1Click(Sender: tObject);
begin
  ImprimePedido(True,'ORCAMENTOIMAGEMTOTAL');
end;

procedure TFrmPedidoVelho.OrcamentoImagemSomenteTotal2Click(Sender: tObject);
begin
  ImprimePedido(False,'ORCAMENTOIMAGEMTOTAL');
end;

procedure TFrmPedidoVelho.RomaneioM31Click(Sender: tObject);
begin
   ImprimePedido(True,'ROM3');
end;

procedure TFrmPedidoVelho.RomaneioM32Click(Sender: tObject);
begin
  ImprimePedido(False,'ROM3');
end;

procedure TFrmPedidoVelho.AtualizaSaldos;
var
  tmp:Real;
begin
...
procedure TFrmPedidoVelho.DsReservaPedDataChange(Sender: tObject;
  Field: TField);
begin
  AtualizaSaldos;
end;
...
procedure TFrmPedidoVelho.SqlCdsPedidoItemCalcFields(DataSet: TDataSet);
begin
  SqlCdsPedidoItemSequencia.AsInteger := SqlCdsPedidoItem.RecNo;
end;

procedure TFrmPedidoVelho.FormCreate(Sender: tObject);
begin
     SELF.Width:=985;
     SELF.Height:=640;
     vIdPedido:=0;
...
procedure TFrmPedidoVelho.SqlCdsPedidoItemPRD_REFERGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
...
procedure TFrmPedidoVelho.RomaneioBSC1Click(Sender: tObject);
begin
   ImprimePedido(True,'BSC');
end;


procedure TFrmPedidoVelho.OramentoMais1Click(Sender: tObject);
begin
     ImprimePedido(True, 'ORCMAIS');
end;

procedure TFrmPedidoVelho.OramentoMais2Click(Sender: tObject);
begin
     ImprimePedido(False, 'ORCMAIS');
end;

procedure TFrmPedidoVelho.RomaneioPlanilha1Click(Sender: tObject);
begin
