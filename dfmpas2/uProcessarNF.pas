...
begin
    ...
    if (BuscaUmDadoSqlAsString('SELECT PRODUTOS FROM SHAREDB') = 'C') then
      qItemNota.SQL.Add('JOIN PRD0000 pr ON pr.PRD_REFER = it.PRD_REFER AND pr.PRD_STATUS = ''A'' AND pid.emp_codigo = nf.emp_codigo ')
    else
...
//       else
  ...
  begin
    ...
    notaf.NFe.Total.IBSCBSTot.gIBS.vIBS := ((qNota.FieldByName('NF_TOT_PROD').AsFloat / 100) * AliqUF  );
